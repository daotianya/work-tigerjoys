...
!function (window) {
    ...
    InfiniteScroll.prototype.initLoadingTip = function () {
        ...
        var $donetip = $('<div class="list-donetip" style="display:none;">' + _this.options.doneTxt + '</div>')

        _this.$element.append(_this.$loading = $('<div class="list-loading">' + _this.options.loadingHtml + '</div>'));
        _this.$element.append($donetip);
        
        $.data(document.body,_this.options.name+'_donetip',$donetip);
    };
    ...
    InfiniteScroll.prototype.bindScrollEvent = function () {
        ...
        console.log('绑定滑动组件==='+$binder.attr('id'));
        $binder.on('scroll.ydui.infinitescroll', function () {
        	// 释放页面滚动权限
        	util.pageScroll.unlock();
            if (_this.loading || $.data(document.body,_this.options.name+'_isDone')) return;

            var contentTop = isWindow ? $(window).scrollTop() : $binder.offset().top;

            // 当浏览器滚动到底部时，此时 _this.$tag.offset().top 等于 contentTop + contentHeight
            //console.info(_this.$tag.offset().top+'--'+(contentTop + contentHeight + contentHeight / 10));
    ...
    InfiniteScroll.prototype.loadList = function () {
        ...
        if (typeof options.loadListFn == 'function') {
        	...
        	util.pageScroll.unlock();
            // 监听外部获取数据方法，以便获取数据
            options.loadListFn().done(function (listArr, nextPage,page) {
                ...
                if (!nextPage) {
                	...
                	$.data(document.body,_this.options.name+'_donetip').remove().appendTo(_this.$element).show();
                	$.data(document.body,_this.options.name+'_isDone',true);
                }
...
!function (window) {
    ...
    PullRefresh.prototype.init = function () {
        ...
        if (_this.options.initLoad) {
            ...
            typeof _this.options.loadListFn == 'function' && _this.options.loadListFn().done(function () {
            	//_this.data('hasdata',false);
            	//_this.data('isDone',false);
    ...
    PullRefresh.prototype.triggerLoad = function () {
        ...
        touches.loading = true;
       	$.data(document.body,_this.options.name+'_isDone',false);

        typeof _this.options.loadListFn == 'function' && _this.options.loadListFn().done(function (listArr , nextPage,page) {
            // 当请求的数据小于pageSize[每页请求数据数]，则认为数据加载完毕，提示相应信息
            if (!nextPage && (listArr.length > 0 || page > 1)) {
            	//$.data(document.body,_this.options.name+'_donetip').remove().appendTo(_this.$element).show();
            	$.data(document.body,_this.options.name+'_isDone',true);
            } else {
            	$.data(document.body,_this.options.name+'_donetip').remove();
            }
